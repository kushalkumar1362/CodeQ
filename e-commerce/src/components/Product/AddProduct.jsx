...
const AddProduct = () => {
  ...
  const prevData = location?.state?.prevData || null;

  const [formData, setFormData] = useState({
    product_name: prevData?.product_name || '',
    product_description: prevData?.product_description || '',
    product_price: prevData?.product_price || '',
    product_images: [],
    product_quantity: prevData?.product_quantity || '',
  ...
  const submitHandler = async (event) => {
    ...
    if (prevData) {
      formDataToSend.append('product_id', prevData._id);
    }
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_SERVER_URL}/api/${prevData ? "update-product" : "add-new-product"}`,
        formDataToSend,
  ...
  return (
    <div className="container mx-auto flex items-center flex-col justify-center relative top-[16%] p-4">
      ...
      >
        <h1 className="text-3xl font-bold mb-4 text-primary">{prevData ? "Update" : "Add"} Product</h1>
        {
              ...
              {input.name === 'product_description' ? (
                ...
                <div className='flex flex-col'>
                  <input
                    ...
                    name={input.name}
                    onChange={changeHandler}
                    className="border-[3px] border-gray-400 rounded-md p-2 focus:outline-none focus:border-[#009087] w-full cursor-pointer"
                    multiple={input.multiple || false}
                    accept={input.accept || ''}
                    required={!prevData?.product_images?.length}
                  />
                </div>
              ) : (
                <input
